...
BEGIN
    ...
    BEGIN
        ...
        VALUES ('sp_ProcessStarInvoice', @errno, @text);
	...
	msg_loop: LOOP
		...
		UPDATE 송장출력_사방넷원본변환
		   SET 별표1 = '★★★'
		 WHERE 배송메세지 LIKE CONCAT('%', var_관리자메세지, '%');
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 배송메세지 검색으로 별표 입력', loop_updated_rows);
    ...
    UPDATE 송장출력_사방넷원본변환
       SET 위치 = '공3'
     WHERE 위치 = '공2' AND CAST(택배수량 AS UNSIGNED) BETWEEN 20 AND 1000000;
	...
	INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('송장출력_공통박스 테이블 초기화 (TRUNCATE)', 0);
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 송장출력_공통박스 데이터 복사', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 송장출력_공통박스 수량만큼 행 복사', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_공통박스 순번 매기기 1', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_공통박스 주소에 순번 추가', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_공통박스 순번 매기기 2', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_공통박스 수량 1로 변경', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_공통박스 박스주문 위치변환', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_공통박스 품목개수 합산', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_공통박스 (서울낱개/서울박스)', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_공통박스 (공통박스 → 서울박스)', ROW_COUNT());

	/*--================================================================================
	-- 송장구분자가 "(1)" 이 아닌 경우 주문번호를 '1234567890' 으로 변경
    --================================================================================*/
    UPDATE 송장출력_공통박스
       SET 주문번호 = '1234567890'
     WHERE 송장구분자 IS NULL OR 송장구분자 <> '(1)';
    
	INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_공통박스 주문번호(1234567890)', ROW_COUNT());
