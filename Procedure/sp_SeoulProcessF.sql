...
BEGIN
    ...
    CREATE TEMPORARY TABLE temp_sorted_data LIKE 송장출력_서울냉동_최종;
	CREATE TEMPORARY TABLE temp_invoices LIKE 송장출력_서울냉동;
    ...
    INSERT INTO temp_invoices (
        ...
        FROM 송장출력_사방넷원본변환 WHERE 송장구분최종 = '서울낱개';
		
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] temp_invoices, 서울냉동낱개 데이터 분류', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] (서울냉동) 송장구분자', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] (서울냉동) 송장구분자*수량', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] (서울냉동) 주소, 수취인명, 택배수량', ROW_COUNT());
	
    /*--================================================================================
	-- (서울냉동) 택배수량1 올림 처리  ('낱개' 로 남아있는 행이 있음)
    --================================================================================*/		
    UPDATE temp_invoices SET 택배수량1 = CEIL(IFNULL(택배수량1, 1));

    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] (서울냉동) 택배수량1', ROW_COUNT());	
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] (서울냉동) 송장구분', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] (서울냉동) 송장구분(단일, 1장)', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] (서울냉동) 품목개수', ROW_COUNT());
	
    /*--================================================================================
	-- (서울냉동) 서울냉동 추가송장 테이블로 유니크 주소 행 이동
    --================================================================================
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] (서울냉동) 추가송장 기본 데이터 생성', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] (서울냉동) 수량, 송장명, 옵션명, 품목코드, 주문번호', ROW_COUNT());	
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] (서울냉동) 추가송장 늘리기', ROW_COUNT());	
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] (서울냉동) 추가송장 순번 매기기', ROW_COUNT());	
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] (서울냉동) 추가송장 주소에 순번 추가', ROW_COUNT());
	
    /*--================================================================================
	-- (서울냉동) 서울냉동 테이블 마지막 정리
    --================================================================================*/
	INSERT INTO temp_sorted_data
		...
		FROM (
			...
			SELECT msg1, msg2, msg3, msg4, msg5, msg6, 수취인명, 전화번호1, 전화번호2, 
			       우편번호, 주소, 송장명, 수량, 배송메세지, 주문번호, 쇼핑몰, 품목코드,
                   택배비용, 박스크기, 출력개수, 별표1, 별표2, 품목개수
			  FROM 송장출력_공통박스 WHERE 송장구분최종 = '서울박스'
			UNION ALL
			SELECT msg1, msg2, msg3, msg4, msg5, msg6, 수취인명, 전화번호1, 전화번호2, 
			       우편번호, 주소, 송장명, 수량, 배송메세지, 주문번호, 쇼핑몰, 품목코드,
                   택배비용, 박스크기, 출력개수, 별표1, 별표2, 품목개수
    ...
    INSERT INTO 송장출력_서울냉동_최종 SELECT * FROM temp_sorted_data;
	
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] (서울냉동) 최종 테이블 데이터 통합', ROW_COUNT());
	
    -- =================================================================================
    -- '드라이아이스 추가' 주소 업데이트
    -- =================================================================================
	UPDATE 송장출력_서울냉동_최종 AS d
		JOIN (
			SELECT
				IFNULL(전화번호1, '') AS 전화번호1, IFNULL(전화번호2, '') AS 전화번호2,
				IFNULL(우편번호,  '') AS 우편번호, IFNULL(수취인명,  '') AS 수취인명,
				MAX(주소) AS 주소
			FROM 송장출력_서울냉동_최종
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 드라이아이스 추가건 주소업데이트', ROW_COUNT());
