...
BEGIN
    ...
    INSERT INTO 송장출력_서울냉동추가 
         ...
         SELECT msg1, msg2, msg3, msg4, msg5, msg6, 수취인명, 전화번호1, 전화번호2, 우편번호, 
           ...
           FROM 송장출력_서울냉동변환
          WHERE 송장구분 = '추가';
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 서울냉동 추가 데이터 분류', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 추가송장 대상 데이터 추출', ROW_COUNT());
	...
	SELECT t.msg1, t.msg2, t.msg3, t.msg4, t.msg5, t.msg6, t.수취인명, t.전화번호1, t.전화번호2, t.우편번호, 
	 ...
	 WHERE t.택배수량1 > 2;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 추가송장 개수만큼 행 복사', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 원본+추가송장 데이터 통합', ROW_COUNT());
    ...
    INSERT INTO 송장출력_서울냉동 
		 ...
		 SELECT msg1, msg2, msg3, msg4, msg5, msg6, 수취인명, 전화번호1, 전화번호2, 우편번호, 
				...
				택배수량1, 택배수량합산, 송장구분자, 송장구분, 송장구분최종, 위치, 위치변환 
		   FROM 송장출력_서울냉동추가합치기;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 최종 데이터 통합 (UNION ALL)', ROW_COUNT());
    ...
    INSERT INTO 송장출력_서울냉동별표
         SELECT * FROM 송장출력_서울냉동
          WHERE 별표1 <> '' OR 별표2 <> '';
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 별표 데이터 임시 저장', ROW_COUNT());
	
    DELETE FROM 송장출력_서울냉동
          WHERE 별표1 <> '' OR 별표2 <> '';
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[DELETE] 원본 테이블에서 별표 데이터 삭제', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 별표 데이터를 다시 맨 위로 이동', ROW_COUNT());
	...
	INSERT INTO 송장출력_서울냉동_최종
         ...
         SELECT msg1, msg2, msg3, msg4, msg5, msg6, 수취인명, 전화번호1, 전화번호2, 우편번호, 
				...
				택배비용, 박스크기, 출력개수, 별표1, 별표2, 품목개수
           FROM 송장출력_서울냉동;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 최종 테이블로 데이터 이동', ROW_COUNT());
