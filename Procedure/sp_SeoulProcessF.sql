...
BEGIN
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('(서울냉동) 송장출력_서울냉동변환 초기화', 0);
	...
	INSERT INTO 송장출력_서울냉동변환 
         ...
         SELECT msg1, msg2, msg3, msg4, msg5, msg6, 수취인명, 전화번호1, 전화번호2, 우편번호, 
           ...
           FROM 송장출력_사방넷원본변환
          WHERE 송장구분최종 = '서울낱개';
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 송장출력_서울냉동변환, 서울낱개 데이터 분류', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_서울냉동변환, 택배수량으로 송장구분자 계산', ROW_COUNT());
	...
	UPDATE 송장출력_서울냉동변환
       SET 송장구분자 = 송장구분자 * 수량;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_서울냉동변환, 송장구분자에 수량 적용', ROW_COUNT());
	...
	UPDATE 송장출력_서울냉동변환 AS t
      JOIN (
       ...
       SET t.택배수량1 = s.구분자합;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_서울냉동변환, 주소/수취인명별 송장구분자 합산', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_서울냉동변환, 택배수량 올림 처리', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_서울냉동변환, 송장구분 설정 (추가/1장)', ROW_COUNT());
    ...
    UPDATE 송장출력_서울냉동변환 t1
      JOIN (
       ...
       SET t1.송장구분 = '단일'
     WHERE t1.송장구분 = '1장';
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_서울냉동변환, 송장구분 설정 (단일)', ROW_COUNT());
		
    /*--================================================================================
	-- (서울냉동) 서울냉동1장 분류
    --================================================================================*/
	TRUNCATE TABLE 송장출력_서울냉동1장;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('(서울냉동) 송장출력_서울냉동1장 초기화', 0);
	...
	INSERT INTO 송장출력_서울냉동1장 
         ...
         SELECT msg1, msg2, msg3, msg4, msg5, msg6, 수취인명, 전화번호1, 전화번호2, 우편번호, 
           ...
           FROM 송장출력_서울냉동변환
          WHERE 송장구분 = '1장';
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 서울냉동 1장 데이터 분류', ROW_COUNT());
	   
    /*--================================================================================
	-- (서울냉동) 서울냉동 단일 분류
    --================================================================================*/
	TRUNCATE TABLE 송장출력_서울냉동단일;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('(서울냉동) 송장출력_서울냉동단일 초기화', 0);
    ...
    INSERT INTO 송장출력_서울냉동단일 
         ...
         SELECT msg1, msg2, msg3, msg4, msg5, msg6, 수취인명, 전화번호1, 전화번호2, 우편번호, 
          ...
          WHERE 송장구분 = '단일'
		  ORDER BY 위치변환 DESC;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 서울냉동 단일 데이터 분류', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_서울냉동단일, 단일 데이터 품목개수 합산', ROW_COUNT());
		
    /*--================================================================================
	-- (서울냉동) 서울냉동 추가 분류
    --================================================================================*/
	TRUNCATE TABLE 송장출력_서울냉동추가;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('(서울냉동) 송장출력_서울냉동추가 초기화', 0);
    ...
    INSERT INTO 송장출력_서울냉동추가 
         ...
         SELECT msg1, msg2, msg3, msg4, msg5, msg6, 수취인명, 전화번호1, 전화번호2, 우편번호, 
          ...
          WHERE 송장구분 = '추가'
		  ORDER BY 주소 DESC;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 송장출력_서울냉동추가 데이터', ROW_COUNT());
	   
    /*--================================================================================
	-- (서울냉동) 서울냉동추가송장 테이블로 유니크 주소 행 이동
    --================================================================================*/
	TRUNCATE TABLE 송장출력_서울냉동추가송장;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('(서울냉동) 송장출력_서울냉동추가송장 초기화', 0);
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 송장출력_서울냉동추가송장', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_서울냉동추가송장, 추가송장 내용 변경 (+++)', ROW_COUNT());
	...
	SELECT t.msg1, t.msg2, t.msg3, t.msg4, t.msg5, t.msg6, t.수취인명, t.전화번호1, t.전화번호2, t.우편번호, 
	 ...
	 WHERE t.택배수량1 > 2;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 송장출력_서울냉동추가송장, 추가송장 늘리기', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_서울냉동추가송장, 추가송장 순번 매기기', ROW_COUNT());
		
    /*--================================================================================
	-- (서울냉동) 서울냉동추가송장 주소업데이트
    --================================================================================*/
    UPDATE 송장출력_서울냉동추가송장 SET 주소 = CONCAT(주소, 송장구분자);
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[UPDATE] 송장출력_서울냉동추가송장, 추가송장 주소에 순번 추가', ROW_COUNT());
	
    /*--================================================================================
	-- (서울냉동) 서울냉동추가 합치기
    --================================================================================*/
    TRUNCATE TABLE 송장출력_서울냉동추가합치기;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('(서울냉동) 송장출력_서울냉동추가합치기 초기화', 0);
	...
	INSERT INTO 송장출력_서울냉동추가합치기
         ...
         SELECT msg1, msg2, msg3, msg4, msg5, msg6, 수취인명, 전화번호1, 전화번호2, 우편번호, 
		   ...
		   ORDER BY 위치 DESC;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 송장출력_서울냉동추가합치기, 원본+추가송장 데이터 통합', ROW_COUNT());

    /*--================================================================================
	-- (서울냉동) 서울냉동 테이블 마지막정리
    --================================================================================*/
    TRUNCATE TABLE 송장출력_서울냉동;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('(서울냉동) 송장출력_서울냉동 초기화', 0);
...
--		  ORDER BY 위치변환 ASC
		 ...
		 SELECT msg1, msg2, msg3, msg4, msg5, msg6, 수취인명, 전화번호1, 전화번호2, 우편번호, 
		   ...
		   FROM 송장출력_서울냉동추가합치기
		   ORDER BY 주소 ASC;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 송장출력_서울냉동, 최종 데이터 통합 (UNION ALL)', ROW_COUNT());
	
    /*--================================================================================
	-- (서울냉동) 별표 행 이동 및 삭제
    --================================================================================*/
    TRUNCATE TABLE 송장출력_서울냉동별표;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('(서울냉동) 송장출력_서울냉동별표 초기화', 0);
	
    INSERT INTO 송장출력_서울냉동별표
         SELECT * FROM 송장출력_서울냉동
          WHERE 별표1 <> '' OR 별표2 <> '';
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 송장출력_서울냉동별표, 별표 데이터 임시 저장', ROW_COUNT());
	
    DELETE FROM 송장출력_서울냉동
          WHERE 별표1 <> '' OR 별표2 <> '';
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[DELETE] 송장출력_서울냉동, 원본 테이블에서 별표 데이터 삭제', ROW_COUNT());
    ...
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 송장출력_서울냉동, 별표 데이터를 다시 맨 위로 이동', ROW_COUNT());
		
    /*--================================================================================
	-- (서울냉동) 송장출력_서울냉동에서 송장출력_서울냉동_최종으로 데이터 이동
    --================================================================================*/
    TRUNCATE TABLE 송장출력_서울냉동_최종;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('(서울냉동) 송장출력_서울냉동_최종 초기화', 0);
	...
	INSERT INTO 송장출력_서울냉동_최종
         ...
         SELECT msg1, msg2, msg3, msg4, msg5, msg6, 수취인명, 전화번호1, 전화번호2, 우편번호, 
				...
				택배비용, 박스크기, 출력개수, 별표1, 별표2, 품목개수
           FROM 송장출력_서울냉동;
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows) VALUES ('[INSERT] 송장출력_서울냉동_최종, 최종 테이블로 데이터 이동', ROW_COUNT());
