...
DROP PROCEDURE IF EXISTS sp_SeoulProcessF_Improved$$

CREATE PROCEDURE sp_SeoulProcessF_Improved()
BEGIN
    /*--================================================================================
	-- 서울냉동 처리 (개선된 오류 처리 버전)
    --================================================================================*/
    ...
    /*--================================================================================
	-- (서울냉동) 서울냉동 데이터 분석 - 상세 검증 포함
    --================================================================================*/
    
    -- 데이터 존재 여부 확인 및 상세 오류 정보 제공
    SET @seoul_frozen_count = (SELECT COUNT(*) FROM 통합송장_서울냉동);
    
    IF @seoul_frozen_count = 0 THEN
        -- 데이터가 없는 경우 상세한 오류 정보 제공
        INSERT INTO sp_execution_log (OperationDescription, AffectedRows, Status, ErrorMessage) 
        VALUES ('[ERROR] 서울냉동 데이터 검증', 0, 'FAILED', '통합송장_서울냉동 테이블에 데이터가 존재하지 않습니다.');
        
        SELECT 
            '서울냉동 데이터가 없습니다.' AS Message,
            'DATA_NOT_FOUND' AS ErrorType,
            '통합송장_서울냉동 테이블에 데이터가 존재하지 않습니다.' AS ErrorDetail,
            '테이블명: 통합송장_서울냉동' AS TableInfo,
            '필요 데이터: 서울냉동 관련 주문 정보' AS RequiredData,
            '해결방법: 서울냉동 데이터를 먼저 입력해주세요.' AS Solution;
            
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = '서울냉동 데이터가 없습니다.';
    END IF;
    
    -- Numbers 테이블 확인
    SET @numbers_count = (SELECT COUNT(*) FROM Numbers);
    
    IF @numbers_count = 0 THEN
        INSERT INTO sp_execution_log (OperationDescription, AffectedRows, Status, ErrorMessage) 
        VALUES ('[ERROR] Numbers 테이블 검증', 0, 'FAILED', 'Numbers 테이블에 데이터가 존재하지 않습니다.');
        
        SELECT 
            'Numbers 테이블에 데이터가 없습니다.' AS Message,
            'DATA_NOT_FOUND' AS ErrorType,
            'Numbers 테이블에 데이터가 존재하지 않습니다.' AS ErrorDetail,
            '테이블명: Numbers' AS TableInfo,
            '필요 데이터: 1부터 시작하는 연속된 숫자 시퀀스' AS RequiredData,
            '해결방법: Numbers 테이블을 생성하고 데이터를 입력해주세요.' AS Solution;
            
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Numbers 테이블에 데이터가 없습니다.';
    END IF;
    ...
    VALUES ('[INSERT] temp_invoices, 서울냉동데이터 분석', ROW_COUNT(), 'SUCCESS');
    ...
    -- 최종 결과를 통합송장_서울냉동_최종 테이블에 삽입
    INSERT INTO 통합송장_서울냉동_최종 (
    ...
    FROM (
        ...
        UNION ALL
        -- (서울냉동) 서울공산 데이터 (기본 테이블)
        SELECT msg1, msg2, msg3, msg4, msg5, msg6, 출고지구분, 전화번호1, 전화번호2, 주문자번호, 주소, 상품명, 수량, 
               상품가격, 주문번호, 주문자, 상품코드, 배송비, 상품크기, 상품수량, 상품설명, 별표1, 별표2, 상품수량합계 
        FROM 통합송장_서울공산 WHERE 출고지구분 = '서울공산'
        UNION ALL
        -- (서울냉동) 서울냉동 1개 배송
        SELECT msg1, msg2, msg3, msg4, msg5, msg6, 출고지구분, 전화번호1, 전화번호2, 주문자번호, 주소, 상품명, 수량, 
               상품가격, 주문번호, 주문자, 상품코드, 배송비, 상품크기, 상품수량, 상품설명, 별표1, 별표2, 상품수량합계 
    ...
    -- =================================================================================
    -- 최종 결과 테이블 주문 중 하나는 '서울냉동 추가' 주문이고 
    --    다른 하나는 일반주문인 경우 주소가 중복되면, '서울냉동 추가' 주문 
    --    주소가 비어있는 주소 필드를 일반주문 주소로 채움.
    -- =================================================================================
    -- 주소 업데이트 처리
    UPDATE 통합송장_서울냉동_최종 AS d
    JOIN (
    ...
    SET d.주소 = src.주소  -- 서울냉동추가 주문의 주소를 업데이트
    WHERE d.상품명 = '서울냉동 추가' AND d.주소 IS NULL;
    
    INSERT INTO sp_execution_log (OperationDescription, AffectedRows, Status) 
    VALUES ('[UPDATE] 서울냉동 추가 주문 주소 업데이트', ROW_COUNT(), 'SUCCESS');
    
    /*--================================================================================
	-- (서울냉동) 통합송장_서울냉동_최종 테이블 기본값 업데이트
    --================================================================================*/
    UPDATE 통합송장_서울냉동_최종, (
        SELECT
