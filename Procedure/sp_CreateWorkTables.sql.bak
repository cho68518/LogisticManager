...
DROP PROCEDURE IF EXISTS sp_CreateWorkTables$$

CREATE PROCEDURE sp_CreateWorkTables()
BEGIN
    /*--================================================================================
	-- 작업용 임시 테이블 생성 - 최종 안정화 버전 ('temp_invoices' 사용)
    --================================================================================*/
    DROP TEMPORARY TABLE IF EXISTS temp_invoices;
    DROP TEMPORARY TABLE IF EXISTS temp_additional_invoices;

    -- 메인 작업용 임시 테이블 (계산을 위해 올바른 숫자 데이터 타입으로 정의)
    CREATE TEMPORARY TABLE temp_invoices (
        ...
        품목개수 INT, -- 숫자 타입
        택배수량 DECIMAL(10,3), -- 숫자 타입 (소수점 계산용)
        택배수량1 DECIMAL(10,3), -- 숫자 타입 (소수점 계산용)
        택배수량합산 DECIMAL(10,3), -- 숫자 타입 (소수점 계산용)
