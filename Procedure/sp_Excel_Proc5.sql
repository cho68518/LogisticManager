...
DROP PROCEDURE IF EXISTS sp_Excel_Proc5$$

CREATE PROCEDURE sp_Excel_Proc5(IN tempTableName VARCHAR(255))
BEGIN
    /*--================================================================================
    -- 엑셀 업로드 데이터 처리
	--    톡딜불가 처리 엑셀파일을 읽어서 테이블 Insert
    --================================================================================*/
    ...
    SET @processSql = CONCAT('
        INSERT INTO 송장출력_톡딜불가 (
            쇼핑몰, 품목코드, 상품명, 필수코드1, 필수코드1상품명, 필수코드2,
            필수코드2상품명, 필수코드3, 필수코드3상품명  
        )
        SELECT 
            쇼핑몰, 
			품목코드, 
